...
from helius.admin import AdminClient, ProjectUsage
...
def test_get_project_usage_sends_api_key_and_project_id():
    route = respx.get(
        "https://admin-api.helius.xyz/v0/admin/projects/project-1/usage"
    ).mock(return_value=httpx.Response(200, json=PROJECT_USAGE_RESPONSE))

    with AdminClient(api_key="test") as client:
        usage = client.get_project_usage("project-1")

    sent = route.calls.last.request
    assert sent.url.params["api-key"] == "test"
    assert sent.url.path == "/v0/admin/projects/project-1/usage"
...
def test_get_project_usage_uses_default_project_id():
    route = respx.get(
        "https://admin-api.helius.xyz/v0/admin/projects/default-project/usage"
    ).mock(return_value=httpx.Response(200, json=PROJECT_USAGE_RESPONSE))

    with AdminClient(api_key="test", project_id="default-project") as client:
        client.get_project_usage()

    assert (
        route.calls.last.request.url.path
        == "/v0/admin/projects/default-project/usage"
    )
