...
def test_set_skips_none_unless_allowed():
    assert RpcRequest(method="test").set("commitment", None).build()["params"] is None
    assert RpcRequest(method="test").set("commitment", None, can_be_none=True).build()[
        "params"
    ] == [{"commitment": None}]
...
def test_config_dict_only_appended_when_non_empty():
    assert RpcRequest(method="test").add("account").build()["params"] == ["account"]
    assert RpcRequest(method="test").set("commitment", "finalized").build()[
        "params"
    ] == [{"commitment": "finalized"}]


def test_params_omitted_when_no_positional_or_config_values():
    payload = RpcRequest(method="test").build()

    assert payload["params"] is None


def test_method_id_and_jsonrpc_are_included():
    payload = RpcRequest(method="test", id="abc", jsonrpc="2.0").build()
